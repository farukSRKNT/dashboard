...
const renderActiveShape = (props, movieCount) => {
  ...
  return (
    <g>
      <text x={cx} y={cy-10} dy={8} textAnchor="middle" fontWeight="bold" fill="#405169">
        {movieCount}
      </text>
...
export default function Categories(props) {
  ...
  const categoriesMovieCount = useSelector(state => state.categoriesMovieCount)
...
return (
  <div className={classes.categoriesWrapper}>
    ...
    <ResponsiveContainer width={204} height={264} className={classes.responsiveContainer}>
      <PieChart >
        <Pie
          activeIndex={0}
          data={categories}
          activeShape={(defaultProps) => renderActiveShape(defaultProps, categoriesMovieCount)}
          innerRadius={75}
          outerRadius={90}
          dataKey="value"
        >
